...
import { getFoodDetail, type FoodDetail } from '@/services/foodService';
...
const fetchFood = async () => {
  try {
    ...
    error.value = null;
    food.value = await getFoodDetail(foodId);
  } catch (err) {
  ...
  <div class="bg-gray-200 min-h-screen flex items-center justify-center font-sans text-gray-800">
    ...
    <div class="relative w-[375px] h-[812px] bg-white shadow-2xl rounded-[35px] overflow-hidden border-[8px] border-gray-800 flex flex-col">
      ...
      <header class="h-14 border-b flex items-center justify-between px-4 bg-white z-20 shrink-0">
        ...
        <h1 class="font-bold text-lg truncate">{{ isLoading ? '로딩 중...' : (food?.foodName || '상세 정보') }}</h1>
        <div class="w-8"></div> 
      </header>
        ...
        <div v-else-if="food" class="bg-white">
          <div class="p-6">
              <div class="flex justify-between items-start mb-2">
                  ...
                  <span class="bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs font-bold">{{ food.category }}</span>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3 mb-8 text-left">
                  <div class="p-4 bg-gray-50 rounded-xl">
                      ...
                      <span class="font-bold text-xl text-gray-800">{{ food.calories }} <span class="text-sm font-normal">kcal</span></span>
                  </div>
                  <div class="p-4 bg-gray-50 rounded-xl">
                      <span class="block text-xs text-gray-500 mb-1">탄수화물</span>
                      <span class="font-bold text-xl text-gray-800">{{ food.carbohydrate }} <span class="text-sm font-normal">g</span></span>
                  </div>
                  <div class="p-4 bg-gray-50 rounded-xl">
                      <span class="block text-xs text-gray-500 mb-1">단백질</span>
                      <span class="font-bold text-xl text-gray-800">{{ food.protein }} <span class="text-sm font-normal">g</span></span>
                  </div>
                  <div class="p-4 bg-gray-50 rounded-xl">
                      <span class="block text-xs text-gray-500 mb-1">지방</span>
                      <span class="font-bold text-xl text-gray-800">{{ food.fat }} <span class="text-sm font-normal">g</span></span>
                  </div>
              </div>
              
              <!-- 수정/삭제 버튼 (isMine이 true일 때만 보임) -->
              <div v-if="food.isMine" class="flex gap-3 mt-10">
                  <button class="flex-1 h-12 border-2 border-red-200 text-red-500 font-bold rounded-xl hover:bg-red-50 transition">
                      삭제
                  </button>
                  <button class="flex-[2] h-12 bg-black text-white font-bold rounded-xl hover:bg-gray-800 transition">
                      수정하기
                  </button>
              </div>
          </div>
